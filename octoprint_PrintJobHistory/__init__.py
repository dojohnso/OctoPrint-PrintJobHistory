...
class PrintJobHistoryPlugin(
    ...
    def _checkForMissingFilamentTracking(self):
        ...
        if ( (self._isSpoolManagerInstalledAndEnabled() == True or self._isFilamentManagerInstalledAndEnabled() == True) and
             (self._settings.get([SettingsKeys.SETTINGS_KEY_SELECTED_FILAMENTTRACKER_PLUGIN]) == SettingsKeys.KEY_SELECTED_NONE_PLUGIN) ):
                ...
                self._logger.warning("Filament tracking is disabled, but some plugins are installed!");
                if (notifyUser):
                    self._sendMessageToClient("notice", "Filament tracking is possible!", "Select a tracking plugin in Print Job History's settings", True)
        else:
            if (self._settings.get([SettingsKeys.SETTINGS_KEY_SELECTED_FILAMENTTRACKER_PLUGIN]) == SettingsKeys.KEY_SELECTED_NONE_PLUGIN):
                if (notifyUser):
                    self._sendMessageToClient("notice", "Filament tracking not possible!",
    ...
    def _addTemperatureToPrintModel(self, printJobModel, bedTemp, toolId, toolTemp):
        ...
        self._logger.info("_addTemperatureToPrintModel")
        self._logger.info(bedTemp)
        self._logger.info(toolId)
        self._logger.info(toolTemp)
        self._logger.info(tempModel)
    ...
    def _capturePrintJobData(self, printStatus, payload):
        ...
        if (captureThePrint == True):
            ...
            self._logger.info("path_on_disk")
            self._logger.info(path_on_disk)
            fileTemps = self.read_temperatures_from_file(path_on_disk)

            self._logger.info("fileTemps")
            self._logger.info(fileTemps)
            # {'bed': 40.0, 'tool0': 230.0}
            self._addTemperatureToPrintModel(self._currentPrintJobModel, fileTemps['bed'], 'tool0', fileTemps['tool0'])

            self._logger.info(self._currentPrintJobModel.allTemperatures)
    ...
    def get_settings_defaults(self):
        ...
        settings[SettingsKeys.SETTINGS_KEY_TAKE_TEMPERATURE_FROM_PREHEAT] = False
        settings[SettingsKeys.SETTINGS_KEY_DELAY_READING_TEMPERATURE_FROM_PRINTER] = 0
